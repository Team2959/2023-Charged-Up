...
public final class Autos {
  ...
  public static SendableChooser<Command> sendableChooser(RobotContainer container) {
    ...
    // sendableChooser.addOption("Test Bump", new BumpCubeCommand(container.m_PlacementArmSubsystem));
    ...
    sendableChooser.addOption("Basic Cube Over Charging Station",
        (new BumpCubeCommand(container.m_PlacementArmSubsystem)
            .andThen(runPath("Basic Cube", container.m_driveSubsystem))));
    sendableChooser.setDefaultOption("Place Cone Auto", placeGamePiece(GamePieceType.Cone, container.m_driveSubsystem, container.m_PlacementArmSubsystem, container.m_IntakeSubsystem));
  ...
  public static Command placeGamePiece(GamePieceType gamePieceType, DriveSubsystem driveSubsystem, PlacementArmSubsystem placementArmSubsystem, IntakeSubsystem intakeSubsystem) {
    ...
    Command readyPiece =
      Commands.sequence(
        ...
        new LineupArmCommand(placementArmSubsystem, ArmPositioningType.Mid),
        new WaitCommand(0.5),
        new ArmVacuumReleaseCommand(placementArmSubsystem),
        new WaitCommand(2)
      );
    return readyPiece.andThen(runPath("Place Game Piece", driveSubsystem)).andThen(new ArmToLoadingCommand(placementArmSubsystem, intakeSubsystem));
  }
